...
\begin{tikzpicture}[automaton, auto, thick]
  ...
  \path[->] (1) edge[color=deterministic, line width=3pt, bend left=15]  node[pos=.8] {$String \cup (Int \cap Odd)$} (2);
  \path[->] (2) edge  node {$Float\cup String$} (3);
  \path[->] (1) edge[color=indeterminate,style=dashed,line width=3pt, bend right=15] node[swap] {$Int \cap \overline{Odd}$} (4);
