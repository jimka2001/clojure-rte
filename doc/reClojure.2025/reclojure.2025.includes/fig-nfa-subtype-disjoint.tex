...
\begin{tikzpicture}[automaton, auto, thick]
  ...
  \path[->] (1) edge[color=deterministic, line width=3pt, bend left=15]  node[pos=.8] {$Long \cap Number$} (2);
  \path[->] (1) edge[color=deterministic, line width=3pt, bend right=15] node[swap] {$Long ~\cap ~!Number$} (3);
  \path[->] (1) edge[color=deterministic, line width=3pt, bend left=15]  node[pos=.8] {$!Long \cap Number$} (4);
  \path[->] (1) edge[color=deterministic, line width=3pt, bend right=15] node[swap] {$!Long~ \cap ~!Number$} (5);
\end{tikzpicture}
\end{document}
