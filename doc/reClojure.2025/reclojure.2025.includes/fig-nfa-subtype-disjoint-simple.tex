...
\begin{tikzpicture}[automaton, auto, thick]
  ...
  \path[->] (1) edge[color=satisfiable, line width=3pt, bend left=15]  node[pos=.8] {$Long\cap Number = Long$} (2);
  \path[->] (1) edge[color=unsatisfiable, dotted, line width=3pt, bend right=15] node[swap] {$Long~\cap~ !Number = \emptyset$} (3);
  \path[->] (1) edge[color=satisfiable, line width=3pt, bend left=15]  node[pos=.8] {$!Long \cap Number$} (4);
  \path[->] (1) edge[color=satisfiable, line width=3pt, bend right=15] node[swap] {$!Long~ \cap~ !Number = ~ !Number$} (5);
\end{tikzpicture}
\end{document}
