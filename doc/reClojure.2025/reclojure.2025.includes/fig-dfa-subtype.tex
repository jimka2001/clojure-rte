...
\begin{tikzpicture}[automaton, auto, thick]
  ...
  \path[->] (1) edge[color=deterministic, line width=3pt, bend left=15]  node[pos=.8] {$Long$} (2);
  \path[->] (2) edge  node {$Double\cup String$} (3);
  \path[->] (1) edge[color=deterministic, line width=3pt, bend right=15] node[swap] {$Number \cap \overline{Long}$} (4);
