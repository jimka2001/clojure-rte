 ...
 \begin{columns}
   \begin{column}{0.4\textwidth}
     \only<2>{
       ...
       We \textcolor{blue}{construct~a  ($\sigma$-DFA) \Emph{Symbolic Deterministic Finite Automaton}}.
       
       \bigskip
       
       \Challenge{3} How to construct a $\sigma$-DFA from an RTE? \Emph{Compile-time}}%
...
\begin{frame}{Deterministic (DFA) vs Non-deterministic (NFA)}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.7\textwidth}
      ...
      Deterministic choice between $Int$ vs $Number\cap\overline{Long}$}%2
      \only<7>{\scalebox{0.95}{\input{fig-dfa-subtype-1-2}}}%
      \only<8>{\scalebox{0.95}{\input{fig-dfa-subtype-2-5}}}%
      \only<9->{\scalebox{0.95}{\input{fig-dfa-subtype}}}%
    \end{column}
  \end{columns}
  \only<9->{\Challenge{4} How to partition/decompose types?\\}
  \only<9>{\quad\textcolor{greeny}{$\{String,Int,Long\}\to\{String,Long,Number\cap\overline{Long}\}$}}%
\end{frame}
