...
\begin{tikzpicture}[automaton, auto, thick]
  ...
  \path[->] (1) edge[color=deterministic, line width=3pt, bend right=15]  node[pos=.7,swap] {$Long \cap Odd$} (2);
  \path[->] (1) edge[color=deterministic, line width=3pt, bend left=15]  node[pos=.8] {$Long~ \cap ~!Odd $} (3);
  \path[->] (1) edge[color=deterministic, line width=3pt, bend left=15]  node[pos=.5,swap] {$!Long \cap Odd$} (4);
  \path[->] (1) edge[color=deterministic, line width=3pt, bend right=15]  node[pos=.5] {$!Long~ \cap~ !Odd$} (5);
\end{tikzpicture}
\end{document}
