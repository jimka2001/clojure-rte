...
## `make-dfa` --- Dfa factory function, which checks consistency.  
...
## `rte-compile` --- Creates a Dfa from an Rte.  See [RTE API](api.md).
## `complete ` --- Render complete the given Dfa.
...
## `minimize` --- Accepts an object of type Dfa, and returns a new object of type Dfa
implementing the minimization of the state machine according to the
Hopcroft minimization algorithm.

## `trim` --- Creates a new Dfa from the given Dfa containing only accessible and co-accessible
...
## `complement` --- Accepts an object of type Dfa, and returns a new object of type Dfa
...
## `extract-rte` --- Accepts an object of type Dfa, and returns a map which associates
...
## `synchronized-product` ---  Assuming that the given Dfas are complete, we compute the syncronized cross product SXP
of the two Dfas.
...
## `synchronized-union` --- Compute the union of two Dfas.  I.e., compute a Dfa which
...
## `synchronized-intersection` --- Compute the intersection of two Dfas. I.e., compute the Dfa which
will recognized any sequence which is recognized by `dfa-1` and also
by `dfa-2`.

## `synchronized-xor` --- Compute the xor of two Dfas. I.e., compute the Dfa which
will recognized any sequence which is recognized by `dfa-1` or
by `dfa-2` but not by both.

## `dfa-equivalent` --- Returns a Boolean indicating whether the two given Dfas
