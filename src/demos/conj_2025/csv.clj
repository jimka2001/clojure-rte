...
(defn write-csv-statistic [gen-rte prefix csv-file-name]
  (let [rte (gen-rte)
    ...
    (letfn [(report [state-count transition-count min-state-count min-transition-count duration-ms]
      ...
      (for [dfa (with-timeout time-out-sec
                  (do (printf "timed out computing rte-to-dfa %s\n" rte)
                      (report nil nil nil nil time-out-sec)
                      nil)
                  [(rte/rte-to-dfa rte)])
            :let [state-count (count (:states dfa))
                  transition-count (reduce + 0 (map :transitions (:states dfa)))]
            min-dfa (with-timeout time-out-sec
                      ...
                      [(xym/minimize dfa)])
            :let [min-state-count (count (:states min-dfa))
                  min-transition-count (reduce + 0 (map :transitions (:states dfa)))
